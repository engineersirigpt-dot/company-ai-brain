...
- [x] **M3 durable terminal retry (`451a91a` → `a3a5503` → `b140b23`) — ✅ Codex GO/SHIP `b140b23` : ENQ orchestration slice HARDENED/COMPLETE (local+synthetic)**
      ...
      **M3c — cross-process crash / exactly-once ข้าม process:** ⏳ EXTERNAL gate — worker บังคับ dedupe เองไม่ได้ ; ปิดเมื่อ provider จริงพิสูจน์ idempotency (key เดิม→ผลเดิม, ไม่ side-effect ซ้ำ) หรือมี durable result checkpoint ก่อน apply
      ...
      → suites: **orchestration 58 · API 73 · migration ALL PASSED** (backlog a+b + F1/F2 rework) ; Codex รันยืนยันเอง 59/59 + probe lone-surrogate→22P02 / NUL→22P05 (class 22 → INVALID_RESULT → FAILED ถูกต้อง)
      ...
      → **non-blocking backlog (Codex, ก่อน provider จริง/production — ไม่ใช่ gate ของ local+synthetic):**
        ✅ (a) lone-surrogate/NUL regression — **CLOSED (Codex GO `9d54fa8`)** (F2: inject ลง **valid field** job_name = otherwise-valid payload → assert exact SQLSTATE surrogate→22P02 / NUL→22P05 + control job_name ปกติ→SUCCEEDED ; เดิม top-level `note` = 22023 unknown-key จึง false-positive)
        ✅ (b) config fail-fast `_validate_config()` — **CLOSED (Codex GO `9d54fa8`)** ; **แก้ latent bug**: `ENQ_APPLY_RETRIES=0` → `range(0)` ข้าม loop → `raise last`(None) → `raise None` ; F1: `math.isfinite()` กัน NaN/±Inf (nan<0/inf<0=False → time.sleep พัง / margin NaN bypass budget≤0) ; validate RETRIES≥1, BACKOFF≥0 finite, POLL_LIMIT≥1, INTERVAL≥0 finite, MARGIN≥0 finite ; tests = subprocess import จริง (พิสูจน์ import-time wiring)
          → deploy-policy note (Codex, ไม่ block PoC): `POLL_INTERVAL_S=0` ไม่ crash แต่ tight loop → ก่อน deploy ตั้ง min `>0` ; sane upper-bounds = production config policy
...
- [x] **V2 (HIGH) — ✅ GO / HARDENED (local+synthetic) — Codex confirm `efab38f`** (migration `010_rfq_signoff_v2.sql`, `61a26f8`→`ee4486a`→`efab38f`)
      ...
      - **gate ก่อนข้อมูลจริง/deploy (Codex, ไม่ block PoC)**: POSIX `[:space:]` ไม่จับ NBSP `U+00A0` / zero-width `U+200B` → NBSP-only/ZWSP-only ยังผ่าน ; ก่อนรับ revoke reason จาก user จริง ต้อง NFKC normalize ที่ API boundary + policy reject invisible/format chars + regression `U+00A0/U+2007/U+202F/U+3000/U+200B` (`normalize(v,NFKC)` แปลง space variants แต่ไม่ลบ `U+200B`)
...
- [x] **V3 (HIGH) — ✅ GO / HARDENED (local+synthetic) — Codex confirm `65e0e96`** (migration `011_rfq_readiness_versioning.sql`, `9dc9466`→`65e0e96`)
      ...
      - `apply_rfq_extraction` insert clarification ตรง ๆ (ไม่ผ่าน add_clarification) จึง**ไม่ bump ซ้ำต่อ clarification** ; หลัง M1 apply สำเร็จ bump aggregate **หนึ่งครั้ง** (ดูบรรทัด M1 ด้านล่าง)
      - **review B1/M1/N1 (`65e0e96`)**: **B1** `add_clarification` เดิม bump ด้วย `raised_by_ref` (provenance, nullable) → `raised_by_ref=NULL` ทำ updated_by_ref NOT NULL พัง (23502) ; แยก authenticated `p_actor` (8-arg, validate) ออกจาก provenance, DROP overload 7-arg
