...
- [x] **M3 durable terminal retry (`451a91a` → `a3a5503` → `b140b23`) — ✅ Codex GO/SHIP `b140b23` : ENQ orchestration slice HARDENED/COMPLETE (local+synthetic)**
      ...
      **M3c — cross-process crash / exactly-once ข้าม process:** ⏳ EXTERNAL gate — worker บังคับ dedupe เองไม่ได้ ; ปิดเมื่อ provider จริงพิสูจน์ idempotency (key เดิม→ผลเดิม, ไม่ side-effect ซ้ำ) หรือมี durable result checkpoint ก่อน apply
      ...
      → **non-blocking backlog (Codex, ก่อน provider จริง/production — ไม่ใช่ gate ของ local+synthetic):**
        ✅ (a) lone-surrogate/NUL regression — CLOSED @ `5087acb`→`9d54fa8` (F2: inject ลง **valid field** job_name = otherwise-valid payload → assert exact SQLSTATE surrogate→22P02 / NUL→22P05 + control job_name ปกติ→SUCCEEDED ; เดิม top-level `note` = 22023 unknown-key จึง false-positive)
        ✅ (b) config fail-fast `_validate_config()` — CLOSED @ `5087acb`→`9d54fa8` ; **แก้ latent bug**: `ENQ_APPLY_RETRIES=0` → `range(0)` ข้าม loop → `raise last`(None) → `raise None` ; F1: `math.isfinite()` กัน NaN/±Inf (nan<0/inf<0=False → time.sleep พัง / margin NaN bypass budget≤0) ; validate RETRIES≥1, BACKOFF≥0 finite, POLL_LIMIT≥1, INTERVAL≥0 finite, MARGIN≥0 finite ; tests = subprocess import จริง (พิสูจน์ import-time wiring)
